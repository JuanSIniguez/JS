...
let listarCategorias = articulo =>{
  articulo.forEach( (e) => {
    if(categorias.includes(e.categoria)){
      console.log("CategoriaRepetida")      
    }else{
...
const pintarMenu = articulo => {
    const categorias = []
    const navMenu = document.querySelector('.dropdown-menu')
    const cardMenu = document.querySelector(".listCategorias").content
    categorias.push(listarCategorias(articulo))
    categorias.forEach( e => {
    cardMenu.querySelector(".dropdown-item").textContent = categorias[e]
    const clone = fragment.cloneNode(true);
    cardMenu.appendChild(fragment)
    console.log(cardMenu)
    console.log(clone)
  } )
  cardMenu.prepend(fragment)  
}
